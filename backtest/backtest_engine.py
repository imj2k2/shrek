        ...
        for idx, date in enumerate(dates):
            ...
            if not trading_halted:
                ...
                for symbol in symbols:
                    ...
                    # The following code would force trade generation for testing.
                    # However, as per user requirements, we want trades based ONLY on real strategy signals
                    # So we're disabling this entirely to ensure trades only happen with valid signals.
                    force_trade_generation = False  # Permanently disabled - trades should come from real signals
                    
                    # This block is kept for reference but will never execute since force_trade_generation is False
                    if force_trade_generation and signal.get('action') == 'hold' and idx % 10 == 0:
                        ...
                        signal = {
                            ...
                            'strategy': self.strategy_name or 'forced_test',  # Use the actual strategy name
